...
import com.fastfoot.scheduler.model.PosicaoClassificacaoContinentalLiga;
...
import com.fastfoot.scheduler.model.factory.CampeonatoMistoCincoLigasFactory;
...
@Service
public class CriarCampeonatosCincoLigas {
	...
	public void criarCampeonatos(Temporada temporada, List<Campeonato> campeonatosNacionais,
		...
		Boolean marcarAmistosos = false; //parametroService.getParametroBoolean(ParametroConstantes.MARCAR_AMISTOSOS_AUTOMATICAMENTE); //TODO
		...
		//
		Map<Liga, List<PosicaoClassificacaoContinentalLiga>> posicoes = getPosicoesClassificacaoContinentalPorLiga(temporada);
		for (int i = 0; i < nroCompeticoes; i++) {
			clubesContinental = new HashMap<Liga, List<Clube>>();

			for (Liga liga : Liga.getAll()) {
				
				NivelCampeonato nc = NivelCampeonato.getContinentalPorOrdem(i);
				
				int posInicial = posicoes.get(liga).stream().filter(p -> p.getNivelCampeonato().equals(nc)).findFirst().get().getPosicaoInicial();
				int posFinal = posicoes.get(liga).stream().filter(p -> p.getNivelCampeonato().equals(nc)).findFirst().get().getPosicaoFinal();
				
				clubesContinental.put(liga, clubeRepository.findByLigaAndAnoAndPosicaoGeralBetween(liga, ano - 1, posInicial, posFinal));
			}

			campeonatoContinental = CampeonatoMistoCincoLigasFactory.criarCampeonato(temporada, clubesContinental, NivelCampeonato.getContinentalPorOrdem(i));
	...
	private Map<Liga, List<PosicaoClassificacaoContinentalLiga>> getPosicoesClassificacaoContinentalPorLiga(Temporada temporada) {
		
		Integer nroCompeticoes = parametroService.getParametroInteger(ParametroConstantes.NUMERO_CAMPEONATOS_CONTINENTAIS);
		Integer numeroClassificadosLiga = 3; //TODO: Parametro...
		
		Map<ClassificacaoContinentalFinal, Clube> campeoesContinentais = clubeService.getCampeoesContinentaisPorAno(temporada.getAno() - 1);
		
		PosicaoClassificacaoContinentalLiga posicaoClassificacaoContinentalLiga = null;
		
		Integer ultimaPosicao = -1;
		
		List<PosicaoClassificacaoContinentalLiga> pccls = null;
		
		Map<Liga, List<PosicaoClassificacaoContinentalLiga>> posicoesLiga = new HashMap<Liga, List<PosicaoClassificacaoContinentalLiga>>();
		
		for (Liga l : Liga.getAll()) {
			
			pccls = new ArrayList<PosicaoClassificacaoContinentalLiga>();
			
			//CI
			posicaoClassificacaoContinentalLiga = new PosicaoClassificacaoContinentalLiga();
			posicaoClassificacaoContinentalLiga.setLiga(l);
			posicaoClassificacaoContinentalLiga.setNivelCampeonato(NivelCampeonato.CONTINENTAL);
			posicaoClassificacaoContinentalLiga.setPosicaoInicial(1);
			
			if (campeoesContinentais.get(ClassificacaoContinentalFinal.C_CAMPEAO).getLiga().equals(l)) {
				posicaoClassificacaoContinentalLiga.setPosicaoFinal(numeroClassificadosLiga + 1);
			} else {
				posicaoClassificacaoContinentalLiga.setPosicaoFinal(numeroClassificadosLiga);
			}
			
			pccls.add(posicaoClassificacaoContinentalLiga);
			ultimaPosicao = posicaoClassificacaoContinentalLiga.getPosicaoFinal();
			
			//CII
			posicaoClassificacaoContinentalLiga = new PosicaoClassificacaoContinentalLiga();
			posicaoClassificacaoContinentalLiga.setLiga(l);
			posicaoClassificacaoContinentalLiga.setNivelCampeonato(NivelCampeonato.CONTINENTAL_II);
			posicaoClassificacaoContinentalLiga.setPosicaoInicial(ultimaPosicao + 1);
			
			if (campeoesContinentais.get(ClassificacaoContinentalFinal.CII_CAMPEAO).getLiga().equals(l)) {
				posicaoClassificacaoContinentalLiga.setPosicaoFinal(ultimaPosicao + numeroClassificadosLiga + 1);
			} else {
				posicaoClassificacaoContinentalLiga.setPosicaoFinal(ultimaPosicao + numeroClassificadosLiga);
			}
			...
			if (nroCompeticoes == 3) {
				posicaoClassificacaoContinentalLiga = new PosicaoClassificacaoContinentalLiga();
				posicaoClassificacaoContinentalLiga.setLiga(l);
				posicaoClassificacaoContinentalLiga.setNivelCampeonato(NivelCampeonato.CONTINENTAL_III);
				posicaoClassificacaoContinentalLiga.setPosicaoInicial(ultimaPosicao + 1);
				
				if (campeoesContinentais.get(ClassificacaoContinentalFinal.CIII_CAMPEAO).getLiga().equals(l)) {
					posicaoClassificacaoContinentalLiga.setPosicaoFinal(ultimaPosicao + numeroClassificadosLiga + 1);
				} else {
					posicaoClassificacaoContinentalLiga.setPosicaoFinal(ultimaPosicao + numeroClassificadosLiga);
				}
